...
def quantize_model_q8(model):
    """Quantize all 2D weight tensors (linear projections) to Q8_0 precision.

    Q8_0 format: scale = max(|block|) / 127, q[i] = round(val[i] / scale)
    Block size: 32 values.

    This simulates Q8_0 precision loss by replacing weights with their
    quantized-then-dequantized versions. PyTorch cannot do native INT8 matmul
    easily, so we degrade the weights to Q8 precision and run inference in F32.
    The results match the Rust INT8 implementation because the math is equivalent.

    Returns: dict with per-layer quality metrics
    """
    quality_report = {}
    ...
    with torch.no_grad():
        for name, param in model.named_parameters():
            ...
            quants = (blocks * inv_scales.unsqueeze(1)).round().clamp(-128, 127)

            # Dequantize: multiply back by scale
            dequantized = quants * scales.unsqueeze(1)
            ...
            # Replace weight with quantized-dequantized version
...
def to_f32_numpy(tensor) -> np.ndarray:
    """Convert any torch tensor (including BF16) to float32 numpy array.

    Also handles HuggingFace model output objects (BaseModelOutputWithPooling,
    BaseModelOutput, etc.) that wrap the actual tensor in named fields.
    """
    # Handle HuggingFace model outputs (BaseModelOutputWithPooling, etc.)
...
def _register_audio_encoder_hooks(model) -> PrefillActivationCapture:
    """Register forward hooks on audio encoder (audio_tower) layers.

    Navigates the model hierarchy to find the audio_tower and its transformer
    layers. Captures each layer output as well as the tower's final output and
    layer-norm-post output (if accessible).

    Returns a PrefillActivationCapture with hooks registered. Caller must
    call capture.remove_hooks() after generate() completes.

    Only meaningful for modes that process audio input (ASR, audio_to_audio).
    """
    capture = PrefillActivationCapture()

    # Navigate to the audio_tower module.
    # For full Qwen3OmniMoe: model.thinker.audio_tower
    # For thinker-only: model.audio_tower
    # Fallback: model.model.audio_tower
    audio_tower = _find_attr(model, [
        "thinker.audio_tower",    # Full Qwen3OmniMoe model
        "audio_tower",            # Thinker-only or Qwen2.5-Omni
        "model.audio_tower",      # Nested model
    ])
    if audio_tower is None:
        logger.warning("Could not locate audio_tower for hooks — skipping audio encoder hooks")
        return capture

    logger.info("Located audio_tower: %s", type(audio_tower).__name__)

    # Register hook on the audio_tower itself to capture the final tower output.
    capture.register("audio_tower_output", audio_tower)
    logger.info("Registered hook on audio_tower (final output)")

    # Layer-norm post (ln_post) — present in Whisper-style encoders.
    ...
    if ln_post is not None:
        ...
        logger.info("audio_tower.ln_post not found (may not exist in this model variant)")

    # Individual transformer layers inside the audio tower.
    # Whisper-style audio encoders store them in .layers
    audio_layers = _find_attr(audio_tower, ["layers"])
    if audio_layers is not None and hasattr(audio_layers, "__len__"):
        for i, layer in enumerate(audio_layers):
            capture.register(f"audio_layer_{i:02d}_output", layer)
        logger.info(
            "Registered hooks on %d audio encoder layers", len(audio_layers)
        )
    else:
        logger.warning("Could not locate audio_tower.layers for per-layer hooks")

    return capture


def _save_audio_encoder_activations(capture: PrefillActivationCapture, save_dir: Path):
    """Save captured audio encoder activations as .npy files.

    Expected keys (subset may be present depending on model structure):
      - audio_tower_output
      - audio_ln_post_output
      - audio_layer_00_output .. audio_layer_NN_output
    """
    if not capture.activations:
        logger.warning("No audio encoder activations were captured by hooks")
        return
    ...
    logger.info(
        "Saved %d audio encoder activation tensors (prefill pass) to %s",
        n_saved, save_dir,
    )


def _register_talker_hooks(model) -> PrefillActivationCapture:
    """Register forward hooks on Talker transformer layers.

    Navigates the model hierarchy to find the talker component and its
    transformer layers. Also hooks the talker norm if present.

    Returns a PrefillActivationCapture with hooks registered. Caller must
    call capture.remove_hooks() after generate() completes.

    Only meaningful for modes that produce audio output (TTS, audio_to_audio).
    """
    capture = PrefillActivationCapture()

    # Navigate to the talker module.
    # For full Qwen3OmniMoe: model.talker
    # May also be at model.model.talker
    talker = _find_attr(model, [
        "talker",         # Full Qwen3OmniMoe model
        "model.talker",   # Nested model
    ])
    if talker is None:
        logger.warning("Could not locate talker for hooks — skipping talker hooks")
        return capture

    logger.info("Located talker: %s", type(talker).__name__)

    # Individual transformer layers inside the talker.
    # Try common paths: talker.layers, talker.model.layers
    ...
    if talker_layers is not None and hasattr(talker_layers, "__len__"):
        ...
        logger.warning("Could not locate talker transformer layers for hooks")

    # Final norm of the talker (talker.norm or talker.model.norm)
    talker_norm = _find_attr(talker, ["norm", "model.norm"])
    if talker_norm is not None:
        capture.register("talker_norm_output", talker_norm)
        logger.info("Registered hook on talker norm")
    else:
        logger.info("talker norm not found (may not exist in this model variant)")
...
def _save_talker_activations(capture: PrefillActivationCapture, save_dir: Path):
    ...
    Expected keys (subset may be present depending on model structure):
      - talker_layer_00_output .. talker_layer_NN_output
      - talker_norm_output
    """
    if not capture.activations:
...
def _run_single_text_variant(
    ...
    """Run a single text prompt variant and save all outputs.

    Args:
        forward_model: The model to use for the forward pass (thinker or full).
        generate_model: The model to use for generation.
        processor: The processor/tokenizer.
        family: Model family string.
        prompt_text: Raw text prompt (before chat template if use_chat_template=True).
        use_chat_template: If True, wrap prompt in chat template before tokenizing.
        save_dir: Directory to save outputs into.
        max_new_tokens: Number of tokens to generate for verification.
    """
    logger.info(
        "  variant: %s  chat_template=%s  prompt=%r",
        save_dir.name, use_chat_template, prompt_text[:60],
    )

    if use_chat_template:
        # Wrap in chat template — produces proper assistant-format response.
        ...
        if hasattr(processor, "apply_chat_template"):
            ...
            formatted = processor.tokenizer.apply_chat_template(messages, **template_kwargs)
        logger.info("  Chat-formatted prompt (first 200 chars): %r", formatted[:200])
        inputs = processor.tokenizer(formatted, return_tensors="pt")
    else:
        # Raw tokenization — no chat template, no system/user framing.
    ...
    # Generation (a few tokens for verification).
    ...
    avg_tok_time = total_gen / n_new if n_new > 0 else 0.0
    logger.info(
...
def run_text_test(
    ...
    """
    Load Thinker only and generate activations for 6 text prompt variants.

    For each of three prompts ("Hello", "The capital of France is",
    "Привет, как дела?") this runs BOTH:
      - raw tokenization (no chat template)   → omni_text_simple/,
                                                omni_text_factual/,
                                                omni_text_russian/
      - chat template wrapping                → omni_text_simple_chat/,
                                                omni_text_factual_chat/,
                                                omni_text_russian_chat/

    This is identical in standalone mode and server mode.
    ...
    Each variant directory saves:
        input_ids.npy
        embedding_output.npy
        thinker_layer_00_output.npy .. thinker_layer_NN_output.npy
        final_norm_output.npy
        logits.npy
        generated_text.txt
    """
    logger.info("=" * 70)
    logger.info("MODE: text_test  (6 variants: 3 prompts x 2 tokenization modes)")
    logger.info("=" * 70)
    ...
    # Three raw prompts x two tokenization modes = 6 variants.
    # (name_suffix, raw_prompt_text)
    raw_prompts = [
        ("simple",  "Hello"),
    ...
    for name_suffix, prompt_text in raw_prompts:
        # --- Raw (no chat template) ---
        raw_dir = output_base / f"omni_text_{name_suffix}{quantize_suffix}"
        logger.info("--- text_test variant: %s (raw) ---", raw_dir.name)
        try:
            _run_single_text_variant(
            ...
            logger.error("FAILED variant %s (raw): %s", name_suffix, exc, exc_info=True)

        # --- Chat template ---
        chat_dir = output_base / f"omni_text_{name_suffix}_chat{quantize_suffix}"
        logger.info("--- text_test variant: %s (chat template) ---", chat_dir.name)
        try:
            _run_single_text_variant(
                forward_model=forward_model,
                generate_model=generate_model,
            ...
            logger.error("FAILED variant %s (chat): %s", name_suffix, exc, exc_info=True)

    log_ram()
    logger.info("text_test complete. Output variants under: %s", output_base)
...
def run_asr_test(
    ...
    save_dir = output_base / f"omni_asr_test{quantize_suffix}"
    ...
    # Register audio encoder hooks to capture per-layer activations from the audio tower.
    # The audio tower processes mel spectrograms before they are injected into the thinker.
    audio_capture = _register_audio_encoder_hooks(model)
    ...
    # Save audio encoder activations captured during generate() prefill pass.
    _save_audio_encoder_activations(audio_capture, save_dir)
...
def run_tts_test(
    ...
    save_dir = output_base / f"omni_tts_test{quantize_suffix}"
    save_dir.mkdir(parents=True, exist_ok=True)
    ...
    # Register talker hooks to capture per-layer activations from the speech decoder.
    # The talker (speech decoder) converts thinker hidden states into audio codec tokens.
    ...
    # Save talker layer activations captured during generate().
...
def run_audio_to_audio_test(
    ...
    save_dir = output_base / f"omni_audio_to_audio{quantize_suffix}"
    save_dir.mkdir(parents=True, exist_ok=True)
    ...
    # Register audio encoder hooks (audio_to_audio uses audio input).
    audio_capture = _register_audio_encoder_hooks(model)

    # Check if model supports audio output
    has_talker = hasattr(model, "talker") or getattr(model, "has_talker", False)

    # Register talker hooks if model has talker (audio_to_audio produces audio output).
    talker_capture = _register_talker_hooks(model) if has_talker else PrefillActivationCapture()
    ...
    # Save audio encoder activations (input audio processing).
    _save_audio_encoder_activations(audio_capture, save_dir)
    audio_capture.remove_hooks()
    audio_capture.clear()

    # Save talker activations (speech output generation).
    _save_talker_activations(talker_capture, save_dir)
    talker_capture.remove_hooks()
    talker_capture.clear()
    ...
    for prompt_name, prompt_text in TEXT_PROMPTS:
        ...
        try:
            _extract_text_activations(
                ...
                save_dir=output_base / f"omni_{prompt_name}{quantize_suffix}",
            )
    ...
    try:
        ...
        try:
            _extract_asr_activations(
                ...
                save_dir=output_base / f"omni_audio_asr{quantize_suffix}",
            )
        except Exception as exc:
            ...
            if _free_full_model:
                ...
                logger.info("Part 2 model freed.")
                log_ram()
        ...
        for tts_name, tts_text in TTS_PROMPTS:
            ...
            try:
                _extract_tts_activations(
                    ...
                    text=tts_text,
                    save_dir=output_base / f"omni_{tts_name}{quantize_suffix}",
                )
            except KeyboardInterrupt:
...
class _OmniRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _handle_run(self):
        ...
        output_base = Path(params["output_dir"]) if params.get("output_dir") else state["output_base"]
        audio_file = params.get("audio_file", state["audio_file"])
        family = state.get("family")

        if params.get("output_dir"):
            logger.info("Using custom output_dir from request params: %s", output_base)
        ...
        try:
            if test_name == "text_test":
                # text_test generates 6 variant directories under output_base;
                # return the base directory as output_dir.
                run_text_test(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        choices=["q8", "q4"],
        help="Quantize model weights before inference. q8=Q8_0 (8-bit), q4=Q4_0 (4-bit)",
    )
    return parser.parse_args()


def _apply_quantization(model, args, output_dir: Path):
    """Apply quantization to model weights in-place if --quantize is set.

    Currently supports q8 (Q8_0 8-bit). Saves a quality report to output_dir.
    ...
    if args.quantize in ("q8", "q4"):
        ...
        print(f"Quantized {len(quality_report)} layers")
        print(f"MSE range: {min(mses):.2e} — {max(mses):.2e}")
        print(f"Cosine sim range: {min(cos_sims):.6f} — {max(cos_sims):.6f}")
...
def main():
    ...
    # Suffix appended to output directory names when quantization is active.
    quantize_suffix = f"_{args.quantize}" if args.quantize else ""
    ...
    try:
        if args.mode == "text_test":
            # Pre-load thinker so we can apply quantization before inference.
            if args.quantize:
                model, processor = load_thinker(model_path)
                family = detect_model_family(model_path)
                _apply_quantization(model, args, output_base)
                run_text_test(model_path, output_base, model=model,
                              processor=processor, family=family,
                              quantize_suffix=quantize_suffix)
            else:
                run_text_test(model_path, output_base,
                              quantize_suffix=quantize_suffix)

        elif args.mode == "asr_test":
            # Pre-load full model so we can apply quantization before inference.
            if args.quantize:
                model, processor = load_full_model(model_path, enable_audio_output=False)
                family = detect_model_family(model_path)
                _apply_quantization(model, args, output_base)
                run_asr_test(model_path, audio_file, output_base, model=model,
                             processor=processor, family=family,
                             quantize_suffix=quantize_suffix)
            else:
            ...
            # Pre-load full model (with audio output) so we can apply quantization.
            if args.quantize:
                model, processor = load_full_model(model_path, enable_audio_output=True)
                family = detect_model_family(model_path)
                _apply_quantization(model, args, output_base)
                run_tts_test(model_path, output_base, model=model,
                             processor=processor, family=family,
                             quantize_suffix=quantize_suffix)
            else:
            ...
            # full_reference loads models in sequence internally; pass quantize_suffix
            # for directory naming. Quantization is not pre-applied here because
            # the mode loads and frees multiple models sequentially.
            # To use quantization with full_reference, run individual modes instead.
            if args.quantize:
                logger.warning(
                    "full_reference mode with --quantize: directory suffixes will be applied, "
                    "but quantization is NOT applied to model weights in this mode. "
                    "Run individual modes (text_test, asr_test, tts_test) with --quantize instead."
                )
